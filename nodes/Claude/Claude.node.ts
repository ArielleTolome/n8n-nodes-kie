...
async function claudeChatRequest(
	...
	const endpointMap: Record<string, string> = {
		'claude-opus-4-6': '/claude-opus-4-6/v1/chat/completions',
		'claude-sonnet-4-6': '/claude-sonnet-4-6/v1/chat/completions',
		'claude-haiku-4-5': '/claude-haiku-4-5/v1/chat/completions',
		'claude-sonnet-4-5': '/claude-sonnet-4-5/v1/chat/completions',
		'claude-opus-4-5': '/claude-opus-4-5/v1/chat/completions',
	};

	const endpoint = endpointMap[model] || `/claude-sonnet-4-6/v1/chat/completions`;
...
export class Claude implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Claude (Kie.ai)',
		name: 'claudeKie',
		icon: 'file:claude.svg',
		group: ['transform'],
		version: 1,
		subtitle: '={{$parameter["model"]}}',
		description: 'Chat with Claude models (Opus 4-6, Sonnet 4-6, Haiku 4-5, etc.) via Kie.ai API',
		defaults: {
			name: 'Claude (Kie.ai)',
		},
		inputs: ['main'],
		...
		properties: [
			{
				...
				type: 'options',
				options: [
					{ name: 'Claude Opus 4-6 (Latest)', value: 'claude-opus-4-6' },
					{ name: 'Claude Sonnet 4-6', value: 'claude-sonnet-4-6' },
					{ name: 'Claude Haiku 4-5 (Fast)', value: 'claude-haiku-4-5' },
					{ name: 'Claude Sonnet 4-5', value: 'claude-sonnet-4-5' },
					{ name: 'Claude Opus 4-5', value: 'claude-opus-4-5' },
				],
				default: 'claude-sonnet-4-6',
				description: 'Claude model to use for chat completions',
				required: true,
			},
			{
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				const response = await claudeChatRequest(
					this,
					model,
					messages,
